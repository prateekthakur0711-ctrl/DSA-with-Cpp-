...
node* searchBST(node* root, int key) {
	if (root == NULL) {
		return NULL; // Tree nhi hai toh key nhi milegi
	}

	if (root->data == key) {
		return root; // root ko return krdo if the data is equals to key
	}
	else if (key < root->data) {
		// Key choti hai toh lst mei hi mill skti hai bas since it is BST
		node* ans = searchBST(root->left, key);
		if (ans == NULL) {
			return NULL; // nhi milla toh return NULL
		}
		else {
			return ans; // mill gaya toh return ans
		}
	}
	else {
		// if key is bigger then go to rst only
		node* ans = searchBST(root -> right, key);
		return ans; // either ans is null or address return whatever you'll get
	}
}
...
int main() {
	...
	node* root = buildBST();
	node* ans = searchBST(root, 40);
	if (ans == NULL) {
		cout << "Not Found\n";
	}
	else {
		cout << "Mill gaya : " << ans->data << endl;
	}
	// preorder(root);
	// cout << endl;
	// inorder(root);
