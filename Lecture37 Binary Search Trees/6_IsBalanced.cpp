...
Pair isBalanced(node* root) {
	...
	if (left.isBalanced == true // LST is Balanced
	        and right.isBalanced == true // RST is balanced
	        and abs(left.height - right.height) <= 1 // Root is also balanced
	   ) {
		p.isBalanced = true; // Entire tree is balanced
	}
	else {
		p.isBalanced = false;
	}
